...
def tum_bist_hisselerini_getir():
    """BIST'teki tüm hisseleri (BIST 30/50/100, Tüm Halka Arzlar ve Diğerleri) dinamik olarak çeker."""
    # BIST'in genel ana hisse ve halka arz havuzunu oluşturan kapsamlı sembol listesi
    # Yatırımcıların en çok işlem yaptığı ve hacim yarattığı tüm evreni kapsar.
    bist_evreni = [
        # BIST 30 & Bankalar & Büyükler
        ...
        "MAVI.IS", "MGROS.IS", "ODAS.IS", "ONCSM.IS", "OYYKÇ.IS", "OYAKC.IS", "Pazarcı", "PENTA.IS", "PETKM.IS", 
        "PGSUS.IS", "QUAGR.IS", "REEDR.IS", "SAHOL.IS", "SASA.IS", "SDTTR.IS", "SISE.IS", "SKBNK.IS", "SMRTG.IS", 
        "SOKM.IS", "TAVHL.IS", "TCELL.IS", "THYAO.IS", "TKFEN.IS", "TMSN.IS", "TOASO.IS", "TSKB.IS", "TTKOM.IS", 
        "TTRAK.IS", "TUPRS.IS", "TURSG.IS", "ULKER.IS", "VAKBN.IS", "VESBE.IS", "VESTL.IS", "YEOTK.IS", "YKBNK.IS", 
        "YYLGD.IS", "ZOREN.IS",
        # Popüler Yeni Halka Arzlar ve Dinamik Hisseler
        "OBAMS.IS", "MEGMT.IS", "BORSK.IS", "CATES.IS", "BRKVY.IS", "TABGD.IS", "ARTMS.IS", "LIDER.IS", 
        "BEGYO.IS", "MEKAG.IS", "SURGY.IS", "KBORU.IS", "MARBL.IS", "ALKLC.IS", "DURKN.IS", "OBAMS.IS",
        "HOROZ.IS", "KTSKR.IS", "VAKFN.IS", "BINHO.IS", "ETILR.IS", "FORMT.IS", "GRTRK.IS", "HUBVC.IS"
    ]
    # Tekrarları önlemek için küme yapıp liste olarak döndürelim
    return list(set(bist_evreni))

def kuresel_ve_kap_haberleri_cek():
    """Küresel finans, FED, yabancı sermaye ve piyasa akışını izler."""
    ...
    try:
        ...
        if feed.entries:
            en_son_baslik = feed.entries[0].title
            haber_ozeti = f"Son Küresel/Yerel Akış: {en_son_baslik}"
    except Exception:
        pass
    return haber_ozeti

def log_kaydet(hisse, fiyat, hedef1, hedef2, stop, durum):
    """Bulunan fırsatları log.csv dosyasına kaydeder (Backtest için)."""
    dosya_adi = "log.csv"
...
def nihai_sinirsiz_sistem():
    print("Bütün BIST Evrenini Kapsayan Mühürlü Scalp Sistemi Başlatılıyor...")
    
    # --- ZAMAN VE SEANS KONTROLÜ (Açılış/Öğle Manipülasyon Filtresi) ---
    turkey_time = datetime.now(timezone(timedelta(hours=3)))
    saat = turkey_time.hour
    dakika = turkey_time.minute
    ...
    if saat == 12 and 30 <= dakika <= 59:
        print("Öğle arası seans arası, bot beklemede.")
        return
    ...
    for hisse in tum_hisseler:
        try:
            ...
            # 1. Teknik Göstergeler (RSI-7 ve MACD Momentum)
            ...
            # 3. ATR Bazlı Dinamik İz Süren Stop (Trailing Stop / Likidite Koruma)
            ...
            # 4. Kademeli Hedefler (Multi-Target Scalp)
            hedef_1 = son_fiyat * 1.03  # %3 İlk Kâr Realizasyonu (Yarısı)
            hedef_2 = son_fiyat * 1.05  # %5 Trend Sürme Hedefi (Kalanı)

            # --- SINIRSIZ MÜHÜRLÜ FİLTRELEME MANTIĞI ---
            kosul_saglandi = (
            ...
            if kosul_saglandi:
                rapor = (
                    f"💎 *TÜM BİST EVRENİ - MÜHÜRLÜ SİNYAL*\n"
                    f"📈 *Hisse:* `{hisse}`\n"
                    ...
                    f"└ 📊 *Gizli Onaylar:* 🔥 Hacim/Sıkışma | 🟢 MACD/RSI Dip Dönüşü"
                )
                bulunan_firsatlar.append(rapor)
                log_kaydet(hisse, round(son_fiyat, 2), round(hedef_1, 2), round(hedef_2, 2), round(dinamik_stop, 2), "Sınırsız Evren Onaylı")
        except Exception as e:
    ...
    if bulunan_firsatlar:
        # En cazip ilk 3 fırsatı gönderelim
        for f in bulunan_firsatlar[:3]:
            telegram_mesaj_gonder(f)
            
        telegram_mesaj_gonder(
            f"🌐 *KÜRESEL PİYASA & TAHTACI RADARI*\n"
            f"📌 *Gelişme:* {piyasa_gundemi}\n"
            f"⚡ *Aksiyon:* Bütün BIST evreni tarandı, en güçlü sinyaller yakalandı!\n"
            f"⏱ *Zaman:* {zaman_str}"
        )
    else:
        telegram_mesaj_gonder(
            f"🧠 *Tüm BIST Evreni Tarama Raporu*\n"
            f"• {piyasa_gundemi}\n"
            f"• Piyasਲardaki tüm hisseler ve halka arzlar tarandı, mühürlü koşullar bekleniyor.\n"
            f"⏱ *Zaman:* {zaman_str}"
        )
